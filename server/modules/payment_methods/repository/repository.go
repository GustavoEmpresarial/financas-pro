// Package repository declara o acesso a dados de payment_methods.
//
...
func New(pool *pgxpool.Pool) *crud.Repo[gen.PaymentMethod] {
	q := gen.New(pool)
	return crud.NewRepo(pool, crud.SQL[gen.PaymentMethod]{
		Table:   Table,
		Columns: Columns,
		List: func(ctx context.Context, userID string) ([]gen.PaymentMethod, error) {
			return q.ListPaymentMethods(ctx, userID)
		},
		Get: func(ctx context.Context, id, userID string) (gen.PaymentMethod, error) {
			return q.GetPaymentMethod(ctx, gen.GetPaymentMethodParams{ID: id, UserID: userID})
		},
		Delete: func(ctx context.Context, id, userID string) (int64, error) {
			return q.SoftDeletePaymentMethod(ctx, gen.SoftDeletePaymentMethodParams{ID: id, UserID: userID})
		},
	})
}
