# payment_methods — vazio de proposito

Este modulo e CRUD puro: as quatro operacoes sao exatamente
`GET` / `POST` / `PUT :id` / `DELETE :id` sobre `payment_methods`, sem nenhuma regra
...
**Quando preencher:** na primeira regra que so valha para payment_methods. Aí este
