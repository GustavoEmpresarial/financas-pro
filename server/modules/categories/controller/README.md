# categories — vazio de proposito

Este modulo e CRUD puro: as quatro operacoes sao exatamente
`GET` / `POST` / `PUT :id` / `DELETE :id` sobre `categories`, sem nenhuma regra
...
**Quando preencher:** na primeira regra que so valha para categories. Aí este
