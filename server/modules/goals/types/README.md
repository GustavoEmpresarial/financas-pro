# goals — vazio de proposito

Este modulo e CRUD puro: as quatro operacoes sao exatamente
`GET` / `POST` / `PUT :id` / `DELETE :id` sobre `financial_goals`, sem nenhuma regra
...
**Quando preencher:** na primeira regra que so valha para goals. Aí este
